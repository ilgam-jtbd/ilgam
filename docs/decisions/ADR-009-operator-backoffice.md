# ADR-009 · 플랫폼 운영자 백오피스 (v2 — QA Queue P0)
...
## MVP 6 스크린 (v2 우선순위 재정렬)

| # | 경로 | 핵심 작업 | 대응 ADR | 우선순위 |
|---|---|---|---|---|
| 1 | `(internal)/qa` | **컨텐츠 QA 큐 (P0 · v2 신설)**. pending 공고 자동 분류 결과 + 운영자 승인/반려. 분당 5~10건 처리 목표 | ADR-010 | **P0** |
| 2 | `(internal)/reports` | 외부 신고 처리: shadow hide → 24h 내 판정 (가짜·미지급·개인정보 트랙 분기) | ADR-008 + ADR-010 §Tier 4 | P0 |
| 3 | `(internal)/employers` | 구인자 승인/반려 큐. `approved_at`·`suspended_at` 토글, 반려 사유 필수 입력 | ADR-005 employers | P1 |
| 4 | `(internal)/workers` | 워커 ban/unban. ban 사유·만료일, `operator_actions` + `audit_log` 이중 기록 | ADR-005 soft delete | P1 |
| 5 | `(internal)` (홈) | 일별 NSM: **MAVJ × MAU × TrustScore** (v2) + 14일 스파크라인 | ADR-002 v2 NSM | P1 |
| 6 | `(internal)/audit` | `audit_log` + `operator_actions` 통합 검색 | 개인정보보호법 제29조 | P2 |
| 7 | `(internal)/ads` (dormant) | M18 트리거 시 활성. 현재 빈 layout만 | ADR-011 | dormant |

**`(internal)/payments` 폐기**: 결제 분쟁 수동 개입은 `(internal)/reports`의 "급여 미지급" 트랙으로 통합 (구인자 신규 게시 차단). 직거래 모델로 PortOne 환불 트리거 자체가 불필요 (옵션 환불 보증은 M2+). v1에서 6번 화면이었으나 v2에서 제거.

**QA 큐가 첫 화면 (홈)**: 운영자 로그인 시 기본 진입 = `(internal)/qa` (대시보드는 `(internal)/dashboard`로 리다이렉트). 이유: QA 처리량이 NSM의 MAVJ를 직접 결정하기 때문에 운영자의 분당 throughput 가시성 우선.
